...
| Agent 独立运行 | 已建骨架 | 35% | 已新增 `src/agent/runner.py`，支持独立初始化和生成决策 |
| Agent API | 已建骨架 | 35% | 已新增 `/api/v1/agent/*` 决策、反馈、交易、持仓、绩效接口 |
| 统一记录模型 | Agent侧已建第一版 | 50% | Agent 自己的 SQLite 已包含决策、反馈、交易、持仓、记忆、报告、绩效快照表 |
| 多环境隔离 | Agent侧已支持 | 45% | Agent 核心表已带 `run_env` |
| 多账户/多组合 | Agent侧已支持 | 40% | Agent 核心表已带 `portfolio_id`、`account_id` |
| 多仓位组 | Agent侧已支持 | 40% | Agent 核心表已带 `position_group_id`，同一 symbol 可分仓统计 |
