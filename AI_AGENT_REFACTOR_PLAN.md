...
| Agent 独立运行 | 已可用 | 65% | 已新增 `src/agent/runner.py`，支持独立初始化和生成建议 |
| Agent API | 已可用 | 65% | 已新增决策、反馈、交易、持仓、绩效、审计、周期总结接口 |
| 统一记录模型 | Agent侧已可用 | 65% | Agent 自己的 SQLite 已包含决策、反馈、交易、持仓、记忆、报告、绩效快照表 |
| 多环境隔离 | Agent侧已支持 | 60% | Agent 核心表已带 `run_env` |
| 多账户/多组合 | Agent侧已支持 | 55% | Agent 核心表已带 `portfolio_id`、`account_id` |
| 多仓位组 | Agent侧已支持 | 55% | Agent 核心表已带 `position_group_id`，同一 symbol 可分仓统计 |
| 模拟盘记录 | 部分已有 | 30% | 需要和真实盘、回测统一表结构 |
| 回测记录归档 | 部分已有 | 40% | 已有 backtest_results，但未统一到订单/成交/持仓链路 |
| 月度/年度统计 | Agent侧第一版已完成 | 45% | 已新增 `src/agent/analyzer.py` 和 `/api/v1/agent/summary`，支持日/周/月/年总结 |
...
- 新增 `src/agent/` 独立包。
- Agent 自有数据库默认位于 `src/agent/data/agent.db`。
- Agent 自有 profile、偏好、特点位于 `src/agent/memory/profile.json`。
- Agent 自有 skill 位于 `src/agent/skills/decision_policy.md`。
- Agent 自有工具位于 `src/agent/tools/`。
- Agent 自有报告目录位于 `src/agent/reports/`。
- 新增 `AgentService`，作为 CLI、API、SDK 和其他系统嵌入调用的统一入口。
- 新增 Agent 决策、反馈、交易、持仓、绩效汇总 API。
- 外部 QuantAITrade 行情/系统数据只通过 `src/agent/adapters/` 读取，Agent 生成的数据全部写入 Agent 自己的库。

### 2026-05-09 决策脑增强

- Agent 从简单均线判断升级为 `multi_factor_v1`。
- 新增多周期行情快照：默认分析 `15m`、`1h`、`4h`、`1d`。
- 新增指标：MA、RSI、MACD、布林带、ATR、支撑压力、成交量倍率、趋势斜率。
- 支持多空双向分析，输出 `long_plan` 和 `short_plan`。
- 最终建议支持 `BUY`、`SELL`、`HOLD`、`REDUCE`、`OBSERVE`。
- 建议包含入场区间、失效位、目标位、盈亏比、风险提示和结构化 `metadata.analysis`。
- Skill 已拆成可调用模块：`skills/registry.py`、`skills/multi_timeframe_strategy.py`、`skills/risk_review.py`。
- Brain 当前负责流程编排，策略细节由 Skill 承担，底层指标计算由 Tool 承担。

### 2026-05-09 学习、审计与交接文档

- 新增 `src/agent/audit.py`，支持按条件审计 Agent 自己的决策、反馈、交易、持仓、记忆和报告。
- 新增 `src/agent/analyzer.py`，支持日、周、月、年周期总结。
- 新增 `/api/v1/agent/audit` 和 `/api/v1/agent/summary`。
- 用户反馈和 Agent 交易记录会进入 `agent_memories`，形成第一版事件记忆。
- 新增 `src/agent/AGENT_DESIGN.md`，作为 Agent 内部设计说明。
- 新增 `src/agent/AGENT_HANDOFF.md`，作为外部系统接入和后续开发交接文档。
